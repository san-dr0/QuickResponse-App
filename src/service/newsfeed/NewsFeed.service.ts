...
export const updateNewsFeed = async (feedID: string, feedThoughts: string) => {
  const record = await firstore()
    .collection(NEWS_FEED_TABLE)
    .doc(feedID)
    .update({feed: feedThoughts});
...
export const createRating = async (
  ratingCount: number,
  ratingMakerUserID: string,
  ratingMakerFullName: string,
  userType: string,
) => {
  // responderRatedID: string, responderRatedFullName: string PARAMS of this function
  const hasRecord = await firstore()
    .collection(RATING_TABLE)
    .where('ratingMakerUserID', '==', ratingMakerUserID)
    .get();
...
export const getCertainRatingCount = async (ratingMakerUserID: string) => {
  const result = await firstore()
    .collection(RATING_TABLE)
    .where('responderRatedID', '==', ratingMakerUserID)
    .get();
